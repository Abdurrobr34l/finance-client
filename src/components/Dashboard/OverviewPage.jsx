...
// } from "recharts";
// import {
//   FaBriefcase, FaCar, FaDownload, FaEllipsisH, FaHome,
//   FaMoneyBillWave, FaPiggyBank, FaReceipt, FaShoppingBag,
//   FaUtensils, FaWallet,
// } from "react-icons/fa";
// import { AuthContext } from "../../Context/AuthContext";
...
// const monthKey = (date) => { const d = new Date(date); return `${d.getFullYear()}-${d.getMonth()}`; };
// const monthLabel = (date) => new Date(date).toLocaleDateString("en-US", { month: "short" });
...
// // ✅ Case-insensitive helpers — works with "Income", "income", "INCOME"
// const isIncomeTx  = (tx) => String(tx?.type || "").toLowerCase() === "income";
// const isExpenseTx = (tx) => String(tx?.type || "").toLowerCase() === "expense";
// const getAmount   = (tx) => Number(tx?.amount || 0);

// // ✅ title (from modal) → note → category as label fallback
// const txLabel = (tx) => tx?.title || tx?.note || tx?.category || "—";

// const categoryIcon = (category = "") => {
//   const v = category.toLowerCase();
//   if (v.includes("work") || v.includes("salary") || v.includes("freelance")) return <FaBriefcase />;
//   if (v.includes("food") || v.includes("dining") || v.includes("restaurant")) return <FaUtensils />;
//   if (v.includes("rent") || v.includes("house") || v.includes("home") || v.includes("housing")) return <FaHome />;
//   if (v.includes("car") || v.includes("transport")) return <FaCar />;
//   if (v.includes("shop") || v.includes("grocery") || v.includes("entertainment")) return <FaShoppingBag />;
//   return <FaEllipsisH />;
// };

// // ✅ description prop added — shows latest transaction label under the value
// const StatCard = ({ title, value, description, note, icon, danger }) => (
//   <div className="rounded-3xl border border-base-300 bg-base-200/55 p-5 shadow-xl shadow-black/10 transition-all duration-300 hover:-translate-y-2 hover:border-accent/40 hover:bg-base-200">
//     <div className="flex items-start justify-between gap-4">
...
//         <h3 className="number-font mt-4 text-2xl font-bold text-primary sm:text-3xl">{value}</h3>
//         {description && (
//           <p className="mt-1 truncate text-xs text-secondary/60" title={description}>{description}</p>
//         )}
//         <p className={`mt-2 text-sm font-medium ${danger ? "text-error" : "text-secondary"}`}>{note}</p>
//       </div>
//       <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-accent/10 text-lg text-accent">
//         {icon}
//       </div>
//     </div>
//   </div>
...
//     const interval = setInterval(fetchTransactions, 7000);
//     window.addEventListener("focus", fetchTransactions);
//     return () => { clearInterval(interval); window.removeEventListener("focus", fetchTransactions); };
//   }, [fetchTransactions]);

//   const dashboard = useMemo(() => {
//     const sorted         = [...transactions].sort((a, b) => new Date(b.date) - new Date(a.date));
//     const anchorDate     = sorted[0]?.date ? new Date(sorted[0].date) : new Date();
//     const activeMonthKey = monthKey(anchorDate);
//     const currentMonth   = transactions.filter((t) => monthKey(t.date) === activeMonthKey);

//     // ✅ All filtering now uses case-insensitive helpers
//     const totalIncome  = transactions.filter(isIncomeTx).reduce((s, t) => s + getAmount(t), 0);
//     const totalExpense = transactions.filter(isExpenseTx).reduce((s, t) => s + getAmount(t), 0);
//     const monthIncome  = currentMonth.filter(isIncomeTx).reduce((s, t) => s + getAmount(t), 0);
//     const monthExpense = currentMonth.filter(isExpenseTx).reduce((s, t) => s + getAmount(t), 0);
//     const balance      = totalIncome - totalExpense;
//     const savingsRate  = monthIncome > 0 ? ((monthIncome - monthExpense) / monthIncome) * 100 : 0;

//     // ✅ Description for Income & Expense stat cards (most recent of each type)
//     const latestIncome  = sorted.find(isIncomeTx);
//     const latestExpense = sorted.find(isExpenseTx);
//     const incomeDescription  = latestIncome  ? `Latest: ${txLabel(latestIncome)}`  : null;
//     const expenseDescription = latestExpense ? `Latest: ${txLabel(latestExpense)}` : null;

//     const last12 = getLastMonths(anchorDate, 12);
//     let runningBalance = 0;

//     const balanceGrowth = last12.map((month) => {
//       const key = monthKey(month);
//       const inc = transactions.filter((t) => monthKey(t.date) === key && isIncomeTx(t)).reduce((s, t) => s + getAmount(t), 0);
//       const exp = transactions.filter((t) => monthKey(t.date) === key && isExpenseTx(t)).reduce((s, t) => s + getAmount(t), 0);
//       runningBalance += inc - exp;
//       return { month: monthLabel(month), balance: runningBalance };
//     });

//     const monthlyExpenses   = last12.map((month) => {
//       const key = monthKey(month);
//       return transactions.filter((t) => monthKey(t.date) === key && isExpenseTx(t)).reduce((s, t) => s + getAmount(t), 0);
//     });
//     const monthsWithExpense = monthlyExpenses.filter((v) => v > 0);
//     const averageExpense    = monthsWithExpense.length > 0
//       ? monthsWithExpense.reduce((s, v) => s + v, 0) / monthsWithExpense.length : 0;
//     const expenseChange     = averageExpense > 0 ? ((monthExpense - averageExpense) / averageExpense) * 100 : 0;

//     const incomeVsExpense = getLastMonths(anchorDate, 3).map((month) => {
//       const key = monthKey(month);
//       return {
//         month:    monthLabel(month),
//         income:   transactions.filter((t) => monthKey(t.date) === key && isIncomeTx(t)).reduce((s, t) => s + getAmount(t), 0),
//         expenses: transactions.filter((t) => monthKey(t.date) === key && isExpenseTx(t)).reduce((s, t) => s + getAmount(t), 0),
//       };
//     });

//     const categoryMap = currentMonth.filter(isExpenseTx).reduce((map, t) => {
//       const cat = t.category || "Other";
//       map[cat] = (map[cat] || 0) + getAmount(t);
//       return map;
//     }, {});

//     const spendingCategories = Object.entries(categoryMap)
//       .map(([name, value]) => ({ name, value, percent: monthExpense > 0 ? Math.round((value / monthExpense) * 100) : 0 }))
//       .sort((a, b) => b.value - a.value);

//     return {
//       balance, monthIncome, monthExpense, savingsRate, expenseChange,
//       balanceGrowth, incomeVsExpense, spendingCategories,
//       incomeDescription, expenseDescription,
//       recentTransactions: sorted.slice(0, 5),
//       activeMonthName: anchorDate.toLocaleDateString("en-US", { month: "long", year: "numeric" }),
//     };
//   }, [transactions]);

//   const exportToCSV = () => {
//     if (transactions.length === 0) return;
//     const headers = ["Date", "Type", "Category", "Title", "Note", "Amount"];
//     const rows = [...transactions]
//       .sort((a, b) => new Date(b.date) - new Date(a.date))
//       .map((t) => [formatDate(t.date), t.type, t.category || "", t.title || "", t.note || "", Number(t.amount || 0).toFixed(2)]);
...
//               onClick={exportToCSV}
//               disabled={transactions.length === 0}
//               className="btn rounded-lg border border-accent bg-accent text-primary transition-all duration-300 hover:bg-transparent hover:text-accent disabled:cursor-not-allowed disabled:opacity-40"
//             >
//               <FaDownload />
//               Export to CSV
//             </button>
...
//         {/* ── Error / Auth banners ── */}
//         {error && (
//           <div className="mb-6 rounded-2xl border border-error/20 bg-error/10 p-4 text-sm text-error">{error}</div>
//         )}
//         {!uid && (
//           <div className="mb-6 rounded-2xl border border-warning/20 bg-warning/10 p-4 text-sm text-warning">
//             Please login first. The overview needs your user ID to load transaction data.
...
//             title="Total Balance"
//             value={money(dashboard.balance)}
//             note={dashboard.balance >= 0 ? "Your net balance is positive" : "Your expenses exceed income"}
//             icon={<FaWallet />}
//             danger={dashboard.balance < 0}
//           />
//           <StatCard
...
//             note="Income tracked this month"
//             icon={<FaMoneyBillWave />}
//           />
//           <StatCard
//             title={`${dashboard.activeMonthName} Expenses`}
//             value={money(dashboard.monthExpense)}
//             description={dashboard.expenseDescription}
//             note={`${dashboard.expenseChange >= 0 ? "+" : ""}${dashboard.expenseChange.toFixed(1)}% vs avg`}
//             icon={<FaReceipt />}
//             danger={dashboard.expenseChange > 0}
//           />
...
//               <p className="mt-1 text-sm text-secondary/70">Asset performance over the last 12 months</p>
//             </div>
//             <div className="rounded-full bg-accent/10 px-4 py-2 text-xs font-bold text-accent">1Y</div>
//           </div>
//           <div className="h-80">
//             <ResponsiveContainer width="100%" height="100%">
//               <AreaChart data={dashboard.balanceGrowth}>
...
//                 </defs>
//                 <CartesianGrid stroke="var(--color-base-300)" strokeDasharray="3 3" vertical={false} />
//                 <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: "var(--color-secondary)", fontSize: 12 }} />
//                 <YAxis hide />
//                 <Tooltip formatter={(value) => [money(value), "Balance"]} contentStyle={tooltipStyle} />
//                 <Area
//                   type="monotone" dataKey="balance"
//                   stroke="var(--color-accent)" strokeWidth={4} fill="url(#balanceGradient)"
//                   dot={{ r: 4, strokeWidth: 2, fill: "var(--color-base-100)", stroke: "var(--color-accent)" }}
//                   activeDot={{ r: 7 }}
//                 />
//               </AreaChart>
...
//             <div className="mt-4 flex justify-center gap-6 text-sm">
//               <span className="flex items-center gap-2 text-secondary">
//                 <span className="h-3 w-3 rounded-full bg-accent" /> Income
//               </span>
//               <span className="flex items-center gap-2 text-secondary">
//                 <span className="h-3 w-3 rounded-full" style={{ backgroundColor: "#7C3AED" }} /> Expenses
//               </span>
//             </div>
//           </div>
...
//             <p className="mb-6 text-sm text-secondary/70">Expense split for {dashboard.activeMonthName}</p>
//             {dashboard.spendingCategories.length === 0 ? (
//               <EmptyCard title="No expense data yet" message="Add expense transactions to see your category breakdown." />
//             ) : (
//               <div className="grid items-center gap-6 md:grid-cols-[1fr_0.9fr]">
//                 <div className="relative h-70">
//                   <ResponsiveContainer width="100%" height="100%">
//                     <PieChart>
//                       <Pie data={dashboard.spendingCategories} dataKey="value" nameKey="name" innerRadius={70} outerRadius={105} paddingAngle={4}>
//                         {dashboard.spendingCategories.map((entry, index) => (
//                           <Cell key={entry.name} fill={chartColors[index % chartColors.length]} />
//                         ))}
//                       </Pie>
...
//                   <div className="pointer-events-none absolute inset-0 grid place-items-center text-center">
//                     <div>
//                       <p className="number-font text-2xl font-black text-primary">{shortMoney(dashboard.monthExpense)}</p>
//                       <p className="text-xs font-bold uppercase tracking-[0.2em] text-secondary/60">Total</p>
//                     </div>
//                   </div>
//                 </div>
//                 <div className="space-y-3">
//                   {dashboard.spendingCategories.map((item, index) => (
//                     <div key={item.name} className="flex items-center justify-between gap-4 rounded-2xl bg-base-300/40 p-3">
//                       <div className="flex items-center gap-3">
...
//           {dashboard.recentTransactions.length === 0 ? (
//             <EmptyCard title="No transactions found" message="Once you add income or expense records, they will appear here automatically." />
//           ) : (
//             <div className="overflow-x-auto">
//               <table className="table">
//                 <thead className="bg-base-300/40 text-secondary/70">
...
//                 <tbody>
//                   {dashboard.recentTransactions.map((item) => {
//                     const isIncome = isIncomeTx(item);
//                     return (
//                       <tr key={item._id} className="border-base-300/30 hover:bg-base-300/20">
//                         <td className="px-5 py-4">
//                           <div className="flex items-center gap-4">
//                             <div className={`grid h-11 w-11 place-items-center rounded-2xl ${isIncome ? "bg-success/10 text-success" : "bg-base-300/40 text-secondary"}`}>
//                               {categoryIcon(item.category)}
//                             </div>
//                             <div>
//                               {/* ✅ title → note → category fallback */}
//                               <p className="font-bold text-primary">{txLabel(item)}</p>
//                               <p className="text-xs capitalize text-secondary/60">{item.type}</p>
//                             </div>
//                           </div>
//                         </td>
...
//                         </td>
//                         <td className="px-5 py-4 text-sm text-secondary/80">{formatDate(item.date)}</td>
//                         <td className={`number-font px-5 py-4 text-right font-black ${isIncome ? "text-success" : "text-error"}`}>
